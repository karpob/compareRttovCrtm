...
if __name__ == "__main__":
    ...
    #################################################################################################
    crtmTauCoef, _ = readTauCoeffODPS( os.path.join(coefficientPathCrtm,'iasi_metop-a.TauCoeff.bin') )
    ...
    idxOz = np.array([1427,1479,1536,1579,1585,1626,1643,1671])-1

    for i,n in enumerate(profileNames): 
        key = n.replace(" ","_")+'_'
        for s in sensitivities:
            #exec('sValCrtm = crtmOb.{}K[i,:,:]*profilesCRTM.{}[i,:]'.format(s,s))
            exec('sValCrtm = crtmOb.{}K[i,:,:]'.format(s))
